...
class LLMService:
    ...
    def __init__(self):
        ...
        self.provider_sessions: Dict[
            str, List[Dict[str, str]]
        ] = {}  # For other providers history
    ...
    async def get_gemini_client(self) -> GeminiClient:
        ...
        async with self._init_lock:
            if self.gemini_client is None:
                ...
                print(
                    f"[LLMService] Initializing GeminiClient with explicit cookies (PSID: {psid[:10]}...)"
                )

                # Create client with EXPLICIT cookies only
                self.gemini_client = GeminiClient(psid, psidts, proxy=None)
    ...
    def get_agent(self, session_id: str) -> CodingAgent:
        ...
        if session_id not in self.agents:
            self.agents[session_id] = CodingAgent(
                workspace_path=self.workspace_path, session_id=session_id
            )
    ...
    async def get_gemini_chat_session(
        self, session_id: str, history: Any = None, fresh: bool = False
    ):
    ...
    def _clean_response_text(self, text: str, tool_call_raw: str = None) -> str:
        ...
        # Remove incomplete JSON blocks at the end of the text (e.g., if stream aborted)
    ...
    async def _send_with_retry(
        ...
        if provider != "gemini":
            raise RuntimeError(
                "_send_with_retry is Gemini-only; use generate_stream for other providers"
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(
                    f"[LLMService] Attempt {attempt + 1}/{max_retries} Failed: {error_str}"
                )

                if "invalid response" in error_str or "failed to generate" in error_str:
                    if session_id and attempt < max_retries - 1:
                        print(
                            f"[LLMService] Detected dead session {session_id}. Recovering with fresh chat..."
                        )
                        try:
                            current_chat = await self.get_gemini_chat_session(
    ...
    async def generate_response(
        ...
        try:
            ...
            if provider_name == "gemini":
                chat_session = await self.get_gemini_chat_session(
            ...
            if agent and self.workspace_path:
                ...
                while iteration < max_iterations:
                        ...
                        if hasattr(gemini_resp, "images") and gemini_resp.images:
                            for img in gemini_resp.images:
                                ...
                                if img_url and img_url not in images:
                                    ...
                                    img_type = (
                                        "generated"
                                        if "generated" in type(img).__name__.lower()
                                        else "web"
                                    )
                        ...
                        async for chunk in provider_svc.generate_stream(
                            ...
                            if "error" in chunk:
                                ...
                                message_parts.append(
                                    {"type": "error", "content": chunk["error"]}
                                )
                                continue
                            if "thought" in chunk:
                    ...
                    if provider_name == "gemini":
                        embedded_thinking, clean_response = self._separate_thinking(
                            response_text
                        )
                        ...
                        if all_thoughts:
                            ...
                            message_parts.append(
                                {"type": "thought", "content": all_thoughts}
                            )
                    else:
                        ...
                        if api_thoughts:
                            message_parts.append(
                                {"type": "thought", "content": api_thoughts}
                            )
                    ...
                    try:
                        ...
                        message_parts.append(
                            {"type": "tool_result", "content": error_msg}
                        )
            ...
            error_str = str(e).lower()
            if (
                "invalid response" in error_str
                or "403" in error_str
                or "failed to generate" in error_str
            ):
    ...
    async def _generate_simple_response(
        ...
        if provider_name == "gemini":
            gemini_resp = await self._send_with_retry(
                chat_session, full_prompt, files=files, session_id=session_id
            )
            response_text = gemini_resp.text or ""
            api_thoughts = getattr(gemini_resp, "thoughts", None) or ""
            ...
            async for chunk in provider_svc.generate_stream(
                messages, self.config.get("model", ""), proxy=self.config.get("proxy")
            ):
                if "error" in chunk:
            ...
            if not accumulated_text and not accumulated_thought:
                ...
                message_parts.append(
                    {"type": "thought", "content": accumulated_thought}
                )
    ...
    async def _handle_image_generation(
        ...
        image_prompt = (
            f"Generate an image: {prompt}. Use your image generation capabilities."
        )
        try:
            image_response = await self._send_with_retry(
                chat_session, image_prompt, session_id=session_id
            )
        except Exception as e:
        ...
        for img in image_response.images:
            ...
            img_type = (
                "generated" if "generated" in type(img).__name__.lower() else "web"
            )
            img_svc = get_image_service(self.workspace_path)
            ...
            if save_to_project and self.workspace_path:
                success, save_path = await img_svc.save_image_from_url(
                    img_url, filename
                )
                if success:
        ...
        return (
            "\n".join(results)
            if results
            else "Image generation completed with no results."
        )
...
Execute this task autonomously and provide a complete summary of what you accomplished."""
            ...
            if provider_name == "gemini":
                ...
                chat = client.start_chat(model=model)
                response = await asyncio.wait_for(
                    chat.send_message(prompt), timeout=120
                )
            ...
            for _ in range(max_iterations):
                ...
                if provider_name == "gemini":
                    response = await asyncio.wait_for(
                        chat.send_message(tool_result), timeout=120
                    )
                    response_text = response.text or ""
                else:
                    break
